...
URL and query params stay bidirectionally synced: editing param rows rewrites the URL through `replaceQuery` (in `template.ts`, preserving any `#hash`), and every keystroke in the URL field re-derives rows via `parseParams`, reusing existing row ids so React keys and descriptions survive — and so the field's `EditorView` is not destroyed and rebuilt per character.

### Template variables

`{{name}}` placeholders resolve against the environment picked **for the collection the request belongs to**, and the split across four files is the same tiering `i18n/` uses. `template.ts` is a **pure leaf**: the `VARIABLE` regex, `variableMap`, `resolverFor`, `templateSpans` (the one classifier both the editor decoration and the static render go through), `resolveUrl` and `replaceQuery`. `store.ts` answers the tree-side question — `collectionOf` (memoised on `tree` identity) and `collectionInPlay` — and it has to live there rather than in `environments.ts`, because `setActiveEnvironment` asks it too and `environments.ts` already imports the store. `environments.ts` turns that answer into a resolver: `resolveFor`, `secretsFor`, `applicableEnvironmentId`, `subscribeEnvironment`, `useVariables`. `templateEditor.ts` is the only module that knows both plus CodeMirror, and exports one module-scope `templateVariables` extension array so consumers wire nothing.

**The selection is per collection, and resolution is by the request's own collection — not by the rail's.** `RequestTabs` iterates `tabs` unfiltered, and `selectCollection` moves `activeCollectionId` without touching `activeId`, so the active tab can belong to one collection while the rail shows another; keying the send off the rail would make Ctrl+Enter mean something other than what the editor shows. `environmentByCollection: Record<string, string>` lives in `ui.json`, an absent key meaning "no environment" — and it is a `Record` rather than a `Map` because `toPrefsFile`'s output is compared as JSON to decide whether the file is dirty, and a `Map` serialises to `{}`, so nothing would ever be written. The one asymmetry: `collectionInPlay` falls back to the rail for the *interface* question, while `environmentIdFor` has no fallback for the *send* question — an unresolved `{{baseUrl}}` fails visibly, whereas borrowing the rail's environment would point a send at another server's credentials.

`subscribeEnvironment`'s guard is three clauses in a fixed order, and each one is load-bearing: the pool (`environments` identity) first and unconditionally, because a retyped row does not move the derived id; then the five identities `ResolutionState` names, because without that clause `setBody` would reach the listener on every keystroke and it dispatches into a view that is mid-update; then the derived environment id, because `toggleNode` rebuilds `tree` on every folder expand and `selectCollection` moves the rail under a request that keeps its own environment.

`VARIABLE` is module-level with `/g`, so **never call `.test()` on it** — `replace` and `matchAll` reset `lastIndex` and `test` does not. The fast path is `includes('{{')`. `MatchDecorator` gets `variablePattern()`, a fresh instance, because it drives `lastIndex` itself.

Two classification rules are load-bearing. **A token the caret is inside is never marked unknown:** `closeBrackets` turns `{{` into `{{}}`, so `{{b}}`, `{{ba}}` and `{{bas}}` are all complete matches on the way to `{{baseUrl}}`, and marking them would flash the danger chip on every keystroke. **With no active environment every token is neutral** — that state is one the picker offers deliberately, and a red wall would say something false.

The chip's CSS lives in `styles/codemirror.css` rather than in an `EditorView.theme`, because an unfocused `TemplateInput` cell paints the same two classes with plain spans, outside any editor. It is a translucent plate rather than a fifth syntax hue because every hue is spent — and because the commonest case is a `{{var}}` inside a JSON string, drawn on top of `--color-syntax-string`, which is green.

`components/TemplateInput.tsx` is the one-line field that replaces the URL `<input>` and the key/value cells. It drives a **raw `EditorView`**, not the `@uiw/react-codemirror` wrapper, because that wrapper lists `onChange` in the dependencies of its reconfigure effect and every call site passes an inline arrow — one reconfigure per render, which in a grid is N per character. **At rest a field is not an editor:** it renders coloured spans and constructs the view on first focus, then keeps it. Always-mounting would put a `MutationObserver`, two `ResizeObserver`s, three `IntersectionObserver`s and a `scroll` listener on `.request-panel` behind every cell of every grid.

The completion dropdown is `tooltips({ position: 'absolute', parent: document.body })`, and all three parts are load-bearing: only absolute mode makes CodeMirror divide by its own measured zoom ratio (the compensation `Select.place` writes by hand), and `document.body` is the only way out of `.kv-wrap`, whose `container-type: inline-size` implies `contain: layout` and so makes it the containing block for any `position: fixed` descendant. `tooltipSpace` is overridden because CodeMirror's default compares layout pixels against client-pixel rects.
