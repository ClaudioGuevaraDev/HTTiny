...
const req = (id: string, name: string, method: RequestDocument['method'], url: string, body = ''): RequestDocument => ({
  id, kind: 'http', name, method, url, dirty: false,
...
export const initialDocuments: Record<string, RequestDocument> = {
  ...
  createUser: req('createUser', 'Create User', 'POST', 'https://api.httiny.dev/users', '{\n  "name": "Maya Chen",\n  "email": "maya@example.com",\n  "role": "developer"\n}'),
...
const rnode = (requestId: string): TreeNode => ({ id: `node-${requestId}`, type: 'request', requestId, name: initialDocuments[requestId].name, method: initialDocuments[requestId].method })
export const initialTree: TreeNode[] = [{
  id: 'main', type: 'collection', name: 'HTTiny Demo', expanded: true, children: [
    { id: 'auth', type: 'folder', name: 'Auth', expanded: true, children: [rnode('login'), rnode('refresh')] },
    { id: 'users-folder', type: 'folder', name: 'Users', expanded: true, children: [rnode('users'), rnode('user'), rnode('createUser'), rnode('deleteUser')] },
