...
import { Check, Copy, KeyRound, Layers, Plus, Trash2, X } from 'lucide-react'
import { useT } from '../language'
import { applicableEnvironmentId } from '../environments'
import { collectionInPlay, freshVariable, useAppStore } from '../store'
...
/**
 * The name of the collection whose pick the modal acts on.
 *
 * A selector rather than a hook so the two components that need it read the same thing;
 * collections are always root nodes, so a `find` over the top level is the whole lookup.
 */
const scopeNameIn = (state: ResolutionState): string => {
  const id = collectionInPlay(state)
  return id ? (state.tree.find(node => node.id === id)?.name ?? '') : ''
}

const tabId = (id: string) => `env-tab-${id}`
const panelId = (id: string) => `env-panel-${id}`

/** `env-<stamp>` and not a uuid: `environmentSecretKey` relies on the id holding no colon. */
const mintId = () => `env-${Date.now()}`
...
export function EnvironmentsModal() {
  const open = useAppStore(s => s.environmentsOpen)
  ...
  return (
    ...
    >
      {open && <EnvironmentsBody onDismiss={() => dialogRef.current?.close()} />}
    </dialog>
  )
}

function EnvironmentsBody({ onDismiss }: { onDismiss: () => void }) {
  const { t, plural } = useT()
  const environments = useAppStore(s => s.environments)
  // What the picker is showing: the environment applying to the collection in play. The
  // dialog therefore opens on what you were just looking at.
  const applicable = useAppStore(applicableEnvironmentId)
  const scopeName = useAppStore(scopeNameIn)
  ...
  // Which one is being *edited*, which is not which one is applied to requests. Local,
  // and seeded from the active one because the body only mounts while the dialog is
  // open — the rule the code view's redaction switch already follows. Collapsing the two
  // would mean clicking a tab to look at production silently changed what the next
  // Ctrl+Enter sends.
  const [editingId, setEditingId] = useState<string | null>(applicable ?? environments[0]?.id ?? null)
  const onNavKeyDown = useRovingFocus('[role="tab"]', 'vertical')
  const editing = environments.find(env => env.id === editingId)
  ...
  const duplicate = (env: Environment) => {
    const id = mintId()
    duplicateEnvironment(env.id, id)
    setEditingId(id)
  }

  const remove = (env: Environment) => {
    // The OK/Cancel labels come from the OS, so the question carries the whole meaning
    // and never names a button — the contract the tree's delete confirmation follows.
    if (!window.confirm(t('env.delete.confirm', { name: env.name }))) return
    const index = environments.findIndex(candidate => candidate.id === env.id)
    const next = environments.filter(candidate => candidate.id !== env.id)
  ...
  if (!environments.length) {
    return (
      <div className="env-shell env-shell-empty">
        ...
        <Placeholder
          icon={<Layers size={20} />}
          title={t('env.empty.title')}
          // The one-pass rule is stated here rather than in a tooltip: this is the
          // screen someone reads before they type their first variable.
  ...
  return (
    <div className="env-shell">
      ...
      <div className="env-nav">
        <h2 id="environments-title" className="env-title">
          {t('env.title')}
        </h2>
        <div className="env-list" role="tablist" aria-orientation="vertical" aria-label={t('env.list')} onKeyDown={onNavKeyDown}>
          {environments.map(env => (
            <button
              ...
              role="tab"
              id={tabId(env.id)}
              aria-selected={env.id === editingId}
              aria-controls={panelId(env.id)}
              tabIndex={env.id === editingId ? 0 : -1}
              className={env.id === editingId ? 'active' : ''}
            ...
            >
              ...
              {env.id === applicable && (
                <span className="env-tab-mark" title={scopeName ? t('env.activeIn', { name: scopeName }) : t('env.active')}>
                  <Check size={12} aria-hidden="true" />
                  <span className="sr-only">{scopeName ? t('env.activeIn', { name: scopeName }) : t('env.active')}</span>
                </span>
              )}
            </button>
          ))}
        </div>
        {/* Outside the tablist: its children are tabs, and this is not one. */}
      ...
      {editing && (
        <div className="env-panel" role="tabpanel" id={panelId(editing.id)} aria-labelledby={tabId(editing.id)} tabIndex={-1}>
          <EnvironmentHeader environment={editing} onDuplicate={() => duplicate(editing)} onDelete={() => remove(editing)} />
          <VariableGrid environment={editing} />
        </div>
      )}
...
function EnvironmentHeader({ environment, onDuplicate, onDelete }: { environment: Environment; onDuplicate: () => void; onDelete: () => void }) {
  const { t } = useT()
  const renameEnvironment = useAppStore(s => s.renameEnvironment)
  const setActiveEnvironment = useAppStore(s => s.setActiveEnvironment)
  const isActive = useAppStore(s => applicableEnvironmentId(s) === environment.id)
  const scopeName = useAppStore(scopeNameIn)
  ...
  return (
    <div className="env-header">
      ...
      <input
        ...
        autoComplete="off"
        onChange={e => renameEnvironment(environment.id, e.target.value)}
      />
      {/* Named, because "active" is now a question with an answer per collection — and
          naming it here is also where someone learns the selection is per collection, in
          the one place they are thinking about environments. Falls back to the unscoped
          copy with no collections, where there is nothing to be active *in*, and is
          disabled there for the same reason the picker is: `setActiveEnvironment` would
          be a no-op. */}
      <button type="button" className="env-activate" disabled={isActive || !scopeName} onClick={() => setActiveEnvironment(environment.id)}>
        <Check size={13} aria-hidden="true" />
        {scopeName ? t(isActive ? 'env.activeIn' : 'env.activateIn', { name: scopeName }) : t(isActive ? 'env.active' : 'env.activate')}
      </button>
 ...
 * `EnvironmentVariable[]` would take an `as`; making the grid generic over its row type
 * is the accretion its own comment resists. So the classes are shared and the columns
 * are not: no description, and a lock where it would have been.
 */
function VariableGrid({ environment }: { environment: Environment }) {
  const { t } = useT()
  ...
  const onChange = (next: EnvironmentVariable[]) => setEnvironmentVariables(environment.id, next)
    ...
    <div className="kv-wrap var-grid">
      ...
      {rows.map(row => (
        <div className="kv-row" key={row.id}>
          ...
          <input
            className="technical-input"
            // Still editable, just not readable over a shoulder — the auth panel's
            // password field. These are credentials for the target API, not for HTTiny,
            // so a password manager offering to fill them is noise.
          ...
          />
          {/* A second `role="switch"`, drawn differently or it reads as a duplicate of
              the first. On means the value goes to the OS credential store and never to
              `workspace.json`. */}
          <button
            type="button"
      ...
      {/* The same thing the sidebar footer says, repeated where a lock is actually
          being switched on. */}
