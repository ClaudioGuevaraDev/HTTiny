...
export const mockExecutor: RequestExecutor = {
  async execute(request, signal) {
    ...
    return {
      state: 'success', status, statusText: status === 201 ? 'Created' : status === 204 ? 'No Content' : 'OK',
      time: 124 + request.url.length, size: body ? `${(new Blob([body]).size / 1024).toFixed(1)} KB` : '0 B', body,
      headers: [
...
export const errorCopy: Record<string, [string, string]> = {
  ...
  TIMEOUT: ['Request timed out', 'The server did not respond within the allowed time.'],
  DNS_ERROR: ['DNS lookup failed', 'The hostname could not be resolved.'],
  CONNECTION_REFUSED: ['Connection refused', 'The remote host refused the connection.'],
}
